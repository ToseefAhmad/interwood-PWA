...
const Image = props => {
    ...
    // STATE INITIALIZED WITH IMAGE FOR THE PRODUCT
    
    const [hoverImageState, setHoverImageState] = useState(resource);
    ...
    const actualImage = src ? (
        ...
        <ResourceImage
            ...
            widths={widths}
            image={hoverImageState}
            ratio={ratio}
            showHoverImage={showImage}
            {...rest}
        />
    );
    return (
        // ADDED MOUSE ENTER AND LEAVE EVENTS FOR HOVER IMAGE AND CIRCLE STYLES
        <div onMouseEnter={() => {
            if (showImage && hoverImage) {
                setHoverImageState(hoverImage);
            }
        }}
            onMouseLeave={() => {
                if (showImage && hoverImage) {
                    setHoverImageState(resource);
                }
            }}
            className={`${containerClass} ${showImage ? classes.figure : ''}`}
        >
            <PlaceholderImage
                alt={alt}
                classes={classes}
