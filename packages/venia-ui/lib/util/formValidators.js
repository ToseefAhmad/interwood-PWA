...
export const phoneNumberVaildate = (value, values, length) => {
    if (value && value.toString().length !== length) {
        const message = {
            id: 'validation.hasLengthExactlyPhone',
            defaultMessage: 'Does not have exact number of characters or must be in formate(0301xxxxxxx)',
            value: length
        };
        return message;
    }
...
export const validateEmail = value => {
    ...
    return expression.test(value) === true ? SUCCESS : 'Please enter a valid email address (Ex: johndoe@domain.com).';
};
