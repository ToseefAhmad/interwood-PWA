...
export const useSearchPage = (props = {}) => {
    ...
    let productsData = data != undefined ? data.products.items :[];
    ...
    }, [productsData, setProductsList, tempPageSize]);

    const fetchMore = useCallback(() =>{
        if(productsData.length > products.length ) {
            let tempData = [];
            let tempSize =  tempPageSize +21;
            productsData.forEach((item, index) =>{
                if(index <= tempSize )
                    tempData.push(item);
            });
    ...
    }, [productsData, tempPageSize, setProductsList, setTempPageSize]);



    let isFetchMore = products.length < productsData.length ? true : false;
